    ...
    $(this).on('click', function(){
        ...
        request.onreadystatechange = function(){
            ...
            if(request.status == 200){
                ...
                if(objData.status){
                    document.querySelector("#uniformesCarrito").innerHTML = objData.htmlCarrito;
    ...
    if(cant <= 0){
        ...
        request.onreadystatechange = function(){
            ...
            if(request.status == 200){
                let objData = JSON.parse(request.responseText);
                if(objData.status){
                    let colSuntotal = document.getElementsByClassName(pro)[0];
                    colSuntotal.cells[4].textContent = objData.totalUniforme;
                    document.querySelector("#subTotalCompra").innerHTML = objData.subTotal;
                    document.querySelector("#totalCompra").innerHTML = objData.total;
                }else{
                    swal("", objData.msg, "error");
